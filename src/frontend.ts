...
// import path from 'path'; // TODO : Make sure not needed
import { dateDatestampFormat, timeAndDateDatestampFormat } from './constants.js';
...
const booleans: { [val: string]: boolean } = {
    '1': true, 'on': true,
...
*/
function storeMultiple(parser: ArgumentParser, namespace: Namespace, values: any[], optionString: string | null, args: any[] = [], kwargs: { [name: string]: any } = {}): void {
    args.forEach((arg: string): void => {
        (namespace as any)[arg] = undefined;
    });
    Object.keys(kwargs).forEach((key): void => {
        (namespace as any)[key] = kwargs[key];
    });
}

/** Read a configuration file during option processing.  (Option callback.) */
function readConfigFile(parser: OptionParser, namespace: Namespace, values: any[], optionString: string | null, args: any[] = [], kwargs: { [name: string]: any } = {}): void {
    let newSettings: {} | undefined;
    try {
        newSettings = parser.getConfigFileSettings(values[0]);
    } catch (error) {
        parser.error(error as Error);
    }
    if (newSettings !== undefined) {
        Object.keys(newSettings).forEach((key): void => {
            (namespace as any)[key] = (newSettings as any)[key];
        });
    }
}
...
function validateEncoding(): void {//setting, value, optionParser, configParser?: any, configSection?: any) {
    /*    try:
            codecs.lookup(value)
        except LookupError:
...
*/
function validateEncodingAndErrorHandler(): void {
    /*    if ':' in value:
...
*/
// TODO: MAKE SURE 'path' NOT NEEDED !
// function makeOnePathAbsolute(basePath: string | any, pathArg: string): string {
//     // typed with any because path.resolve accepts undefined.
//     return path.resolve(basePath, pathArg);
// }

// function makePathsAbsolute(pathdict: { [name: string]: any }, keys: string[], basePath?: string): void {
//     let bp = basePath;
...
export class OptionParser extends ArgumentParser {
    ...
    public settingsDefaults: { [propName: string]: any } = {
     ...
     */
    public populateFromComponents(components: (SettingsSpec | undefined)[]): void {
        ...
        components.forEach((component): void => {
            ...
            settingsSpec.forEach((spec: SettingsSpecType): void => {
                const [title, description, optionSpec] = spec;
                optionSpec.forEach(([helpText, optionStrings, optionArgs]): void => {
                    if (optionStrings.indexOf('--help') !== -1) {
                        ...
                        if (a === 'store_const') {
                            ...
                            action = 'validating';
                            newArgs.nargs = optionArgs.nargs, newArgs.validator = optionArgs.validator;
    ...
    public setDefaultsFromDict(defaults: ConfigSettings): void {
