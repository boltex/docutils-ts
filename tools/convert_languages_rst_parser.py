        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Assign):
                    # Handle assignments like 'directives = {...}'
                    for target in node.targets:
                        if isinstance(target, ast.Name):
                            var_name = target.id
                            # Check if it's one of the variables we care about
                            if var_name in ['directives', 'roles']:
                                try:
            ...
            directives = local_scope.get('directives', {})
            roles = local_scope.get('roles', {})

            # Check if we actually found the expected variables
            if not directives and not roles:
                 print(f"  Warning: No expected variables (directives, roles) found or parsed in {filename}. Skipping TS file generation.", file=sys.stderr)
                 error_files += 1
            ...
            ts_directives = format_dict_as_ts_object(directives, indent_level=0)
            ts_roles = format_dict_as_ts_object(roles, indent_level=0)

            # Construct the TypeScript file content
            ts_content = f"""export const directives = {ts_directives};

export const roles = {ts_roles};
"""
